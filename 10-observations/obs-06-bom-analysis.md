...
| **3** | **MS sheets specified two ways — in *both* the body and the lid.** Read directly from the cells 2026-08-31: <br>• **Body** — `MS DRUM` sheet says `0.8 × 920`; `BODY SHEET` sheet says `0.97`, from `CRCA COIL 0.97 × 914` (12.4 kg). <br>• **Lid** — `MS DRUM` sheet says `0.97 × 1315`; `BODY SHEET` sheet says `Lid Sheet 0.9 × 1320 × 655 mm`, from `CRCA COIL 0.9 × 1315` (6.152 kg). <br>• **Lid width, inside one cell** — the sheet is described as **1320 mm wide**, cut from a **1315 mm coil**. **A 1320 mm sheet cannot come from a 1315 mm coil.** Found 2026-08-31 | 🟠 **Three contradictions in one product.** Earlier versions of this row caught only the body sheet, then only the two thicknesses. Steel deduction is wrong on whichever figure is wrong |
...
| **7** | **`CAGE-MAX` books the wrong bar, at the wrong weight — and possibly twice the bars.** Found 2026-08-31, sharpened the same day. Level 2 labels **`1002 = 456 g, "FOR MAX"`** and **`1018 = 463 g, "FOR BIG"`**. <br>• `CAGE-BIG` books 1018 × 20 at **9,260 g** (463 each ✓). <br>• `CAGE-MAX` books 1018 × 20 at **9,120 g** — 456 each, the **1002** weight. **140 g short per cage.** <br>• **`CAGE-MAX` consumes *both* 1018 × 20 *and* 1002 × 20 — 40 vertical bars**, against `CAGE-BIG`'s 20 | 🟠 **Two readings, and it must be asked as an either/or.** Either MAX genuinely takes double the bars, **or the 1018 line was left behind when MAX was copied from BIG, and the wrong weight came with it** — which the `"FOR BIG"` label on 1018 supports. Asking "is 9,120 a typo" gets a yes and leaves the bar count unexamined |
...
11. ⚠️ **`CAGE-MAX` — does it take 40 vertical bars, or was the 1018 line left behind?** Finding 7.
    Level 2 labels 1018 `"FOR BIG"` and 1002 `"FOR MAX"`, yet MAX books **both** — 40 bars against
    BIG's 20 — and the 1018 line carries 1002's weight. **Ask as an either/or**, not as "is this a
    typo": a yes to the typo closes the weight and leaves the bar count live.
11b. **Can a 1320 mm lid sheet come from a 1315 mm coil?** Finding 3. The same cell states both. One
    of the two is wrong, and steel deduction follows whichever is used.
