...
describes **the slice we build and show Pyramid** — **one PRD per demo module**, and the 22 screens
those PRDs need.
...
| 03 | [LR Tracking (UdyogERP)](prd-03-lr-tracking/prd.md) | ⑨ ⑩ | [prd-04](../prd-04-lr-tracking/prd.md) | 2 |
...
All 22 screen specs live in [`screen-specs/`](screen-specs/_index.md), one sub-folder per demo PRD,
...
> read as the Incoterm *Delivered Duty Paid*. `A-DM-01`. If Pyramid meant the Incoterm, this module
...
| Inventory ageing, LR ageing dashboard, pipeline view | prd-01, prd-04 | **Partially in.** Age appears as a **column** on the LR Tracker and Stock by Location. The standalone dashboards are not built |
...
| [LR Tracking](prd-03-lr-tracking/prd.md) | `REQ-LR-001`, `REQ-LR-004`, `REQ-LR-101`, `REQ-LR-102`, `REQ-LR-103`, `REQ-LR-104`, `REQ-LR-105`, `REQ-LR-201`, `REQ-LR-203`, `REQ-LR-303`, `REQ-LR-304` |
...
Three things Pyramid asked for that **no existing PRD carries**. They are requirements, not
re-labellings.

| ID | Requirement | Why it is new | Acceptance criteria |
| -- | ----------- | ------------- | ------------------- |
| `REQ-DM-001` | **Machinery spares is a first-class stock category**, alongside RM, WIP, components, FG, regrind, returned, scrap | `REQ-IV-002` lists seven categories and spares is not one of them. prd-02 treats spares as a Path B *purchase* but never as a *stock class* | Spares carry their own category on every stock screen; a spare can be indented, received on a GRN, and issued to a machine |
...
| `REQ-DM-003` | **Inbound LRs originate in UdyogERP** and are imported, not keyed | prd-04 designs Phlo-native LR capture with `manual`, `api` and `import` sources (`REQ-LR-304`), but no PRD states UdyogERP as the system of record for LRs. Pyramid's module list does | An imported LR shows source `import` and its UdyogERP reference; stage updates after import are made in Phlo and marked `manual` |
...
> *"by how the machines are placed"* and that the incumbent's `Bin No.` / `Rack No.` fields sat blank
...
| `A-DM-04` | UdyogERP can export LR data in some readable form | Never tested. Nobody on this project has seen the LR screen | The demo import becomes a file upload of a spreadsheet the store team maintains by hand |
...
The demo uses the PRDs' entities unchanged, with **three additions** driven by the new requirements.
...
| **LRImport** | id, source_system (`udyog`), source_reference, imported_at, row_count, status | New. `REQ-DM-003` |
...
| `LR_IMPORTED` | A batch of LRs pulled from UdyogERP |
...
ACT 1 — PROCURE TO STOCK
 ...
 ⑨  LR Tracker             8 inbound LRs from UdyogERP, across all five stages
...
22 screens across the 12 PRDs. Full detail in [`screen-specs/`](screen-specs/_index.md); each PRD's
...
| ⑨ | LR Tracker | [03 LR Tracking](prd-03-lr-tracking/prd.md) |
...
4. **What can UdyogERP export for LRs?** `A-DM-04`. Nobody has seen the screen. The import path is
   designed against an assumption about a system we have not opened.
5. **Is stock re-keyed to location for the product, or only for the demo?** The data model delta above
   is small to write and expensive to reverse.
6. **Does the demo end at the truck, or at the invoice?** Invoicing is cut here. If Pyramid expects to
