...
requirements:
  [REQ-FM-001, REQ-FM-002, REQ-FM-004, REQ-FM-005, REQ-FM-007, REQ-FM-008]
---
...
> driver registry **folded in as a picker** rather than a separate screen. Cut: driver and vehicle
> history, cross-plant assignment (`A-FM-05` parks the inter-plant question), and **everything in
> prd-13 Fleet Cost** — no trip cost, no cost-to-serve, no driver advance.

> **Revised 2026-09-09.** The vehicle picker now reads from a standalone screen —
> [Vehicle Registry](../prd-13-vehicle-management/screen-vehicle-registry.md), **beat ㉒**, opened right
> before this one. Driver stays picker-only.
...
`REQ-FM-005` checks availability before assignment. Hiding a truck that is on a trip answers _"where is
GJ-16-XX-4090?"_ with silence; showing it disabled, with the trip it is on, answers the question the
...
_Driver A_, _Driver B_. All real person names were stripped from this project's documents, and several
...
prd-13 Fleet Cost designs trip cost, cost-to-serve and driver advances, and **all of it is cut**. The Class A/B
...
| Label        | Format                    | Source                   | Notes                          |
| ------------ | ------------------------- | ------------------------ | ------------------------------ |
| Driver       | Position label            | `drivers.name`           | _Driver A_ — never a real name |
...
| e-Way Bill | Vehicle number written back to it    | _"Vehicle GJ-16-XX-4102 added to the e-Way Bill."_ |
...
| No vehicle available   | _"No truck free at Unit 7. 2 return today."_ naming the trips — **never a bare empty list**                                     |
...
| **Trip opened**        | Redirect to [Trip Board](screen-trip-board.md), toast: _"TR-2206 open. GJ-16-XX-4102, Driver A."_ — carries the demo to beat ㉔ |
| Error                  | Selection kept, retry offered                                                                                                   |
| Restricted             | _Design intent:_ fleet roles at their own plant. **Not enforced in the demo**                                                   |
