...
> | ❌ 1.3 | ~~₹60–66 lakhs trapped in inventory~~ | Chaitya | **WITHDRAWN 2026-08-21 (RP) — do not use** |
...
| ❌ 1.3 | ~~How much capital is trapped in inventory?~~ **WITHDRAWN 2026-08-21 (RP): the ₹60–66 lakh figure is not to be used anywhere.** No verified figure exists; do not quote one in client-facing material | site-visit |
...
| ⚠️ 2.5 | **PARTLY ANSWERED 2026-08-21 (RP): approval sits at HO** — procurement team, and in some cases promoters/management. **Levels and thresholds deliberately deferred — not needed for the demo** | proc-01 |
...
| #      | Question                                                                                            | Source                       |
| ------ | --------------------------------------------------------------------------------------------------- | ---------------------------- |
| ⚠️ 4.17 | **PARTLY ANSWERED 2026-08-21 (RP): there is no single dominant stage — *all* of them occur, case by case.** No representative consignment exists to measure. **Design implication: Phlo must instrument every stage rather than optimise one.** Original: **Where do the 5–8 days actually go?** Split inbound LR ageing across vendor dispatch, carrier transit, dwell at the carrier facility, and plant-arrival-to-GRN. **Highest-value question in the project** — it decides what Phlo builds first | proc-02, gap-analysis |
...
| ⚪ 10.3 | **CLOSED AS A DEMO BLOCKER 2026-08-21 (RP decision): we will design these screens first-hand rather than mirror UdyogERP's UI.** Still relevant for the eventual CSV import contract, but not gating the demo. Original: **Purchase-side ERP screens have never been seen.** All 12 documented screens are sales-side. The CSV-export integration plan targets a PO screen nobody at Jetbro has examined | as-is model, obs-02 |
...
| ❌ 10.25 | ~~Does ₹60–66 lakhs mean total trapped inventory?~~ **VOID 2026-08-21 — the figure is withdrawn** | extract |
| ⚪ 10.26 | **DOWNGRADED 2026-08-21** — screens will be designed first-hand for the demo. Still worth having for migration/field-mapping later. Original: **Re-capture the ERP screens as images.** The screenshots are not in the repo — only a transcription. Nobody can verify a field, and the purchase-side screens were never captured at all | 00-inbox audit |
...
| ✅ 10.29 | **ANSWERED 2026-08-21 (RP): paper or Excel.** Serial numbers, reject deletions and defect records are therefore **not digital anywhere**. Original: **Where does the "production sheet" live?** Referenced repeatedly across the photographed work instructions — reject serials are deleted from it, defects recorded on it. It is probably the single most important undocumented record in the plant, and nothing in the ERP screens corresponds to it | obs-04 |
...
| ✅ 10.36 | **Where does stock live? ANSWERED 2026-08-21 (RP): all stock, of every kind, is in Excel** — probably distributed per plant. Corroborated hard by the field extract: on the sampled item, Bin No. **(blank)**, Rack No. **(blank)**, Include in Stock Valuation **unchecked**, Re-order Level **0.00**. UdyogERP has stock *fields* but no stock *data*. **Phlo's inventory module replaces spreadsheets, not an ERP module** | RP, extract |
| ⚠️ 10.37 | **How is a production run decided? UNKNOWN (RP, 2026-08-21).** Possibly forecast, running POs, or against SOs — commodity product, so likely mixed. **Demo assumption: production runs against sales orders.** Must be re-tested before build | RP |
| ✅ 10.38 | **Is there any demand planning? ANSWERED 2026-08-21 (RP): no — the promoters' judgement *is* the whole of it.** R2: they weigh market conditions, forward requirement from customer POs, and current stock, then decide. **There is no as-is demand planning to digitise — Phlo would be introducing the capability, not improving one** | RP, R2 |
| ✅ 10.39 | **Demo scope DECIDED 2026-08-21 (RP): Phlo covers the full chain, sales invoice and e-Way Bill included.** No hand-off to UdyogERP in the demo. Implies GST/IRN/TCS/place-of-supply parity is table stakes, not differentiation | RP |
| 🔴 10.40 | **How does a customer order arrive? UNKNOWN (RP, 2026-08-21).** Email, phone, portal, PDF? Who keys it as a Sales Order, and does it sit anywhere first? Demo area 9 has a known screen but no known intake | RP |
| ✅ 10.41 | **Fleet cost tracking: NOTHING is tracked today (RP, 2026-08-21).** Some manual/Excel at best; **nothing on a single portal.** Cost model to build: **attributable to an invoice** — fuel, road tax, driver welfare (food, accommodation, sleeping); **not attributable** — repairs and maintenance, which belong to the vehicle. Phlo *introduces* this capability | RP, rec-32 |
| ✅ 10.42 | **GRN tolerance: configurable and unopinionated for the demo (RP, 2026-08-21).** The PRD's inherited ±2% assumption has no basis and should not be presented as a recommendation | RP |
| ⚠️ 10.43 | **Exports: SCOPED OUT of the demo (RP, 2026-08-21) — but the evidence conflicts and this is *not* recorded as fact.** RP states Pyramid does not export. Against that: a real Delivery Challan in the field extract shows **Export Type = "Without IGST", Place of Supply = "Others", zero GST**, to *Samuda Chemical Complex Ltd*; the Supply Master carries a **RODTEP** (export incentive) field; and finished IBCs carry a ~40-country recollect label plus a `pyramidibccontainers.com` domain. **Scoped out for the demo; must be settled before any full-ERP build** | RP, extract, obs-04 |
| ✅ 10.44 | **Inbound LR capture (demo): received digitally by "the team" — store or purchase, deliberately unspecified (RP, 2026-08-21).** Do not hard-code the owner in the demo; keep the creator role open | RP |
| ✅ 10.45 | **Batch/serial traceability IS in scope (RP, 2026-08-21) — and it is core to the story, not a nicety.** Rationale: a **base product** receives client-specific modifications after manufacture — stickers, paint, valve point, and more. *"Not everything is commoditized."* Strongly corroborated: per-unit serials on IBCs (`PTL-VII-L1-26-H-3493`), in-house screen printing of customer fill data, 42 branded cap-seal SKUs, and rec-32: *"based on the different party's specification, they will modify the final finished goods… valve change… cage change or pallet change."* **Data model needs base SKU + post-production modification layer, tracked per serial** | RP, obs-04, rec-32 |
| ✅ 10.46 | **Demo footprint: TWO plants, both under the SAME GSTIN (RP, 2026-08-21).** Deliberately avoids inter-unit invoicing. Consistent with rec-32: *"direct challan is created for transport from one plant to another if they are in same GST"* — so plant-to-plant movement in the demo is a **delivery challan**, not a sale-purchase invoice | RP, rec-32 |
| ✅ 10.47 | **Dispatch sequencing (RP, 2026-08-21): the dispatch person picks which sales orders ship today, or works a list, driven by delivery due date and SO creation date.** Gives Phlo a demonstrable prioritisation rule for the dispatch queue | RP |
| ✅ 10.48 | **Production consumes raw material via BOM — REQUIRED for the demo (RP, 2026-08-21).** A completed run must deduct HDPE (and components) from stock. **This makes a real BOM a hard dependency for demo area 7** | RP |
| ⏳ 10.49 | **BOMs incoming (RP, 2026-08-21): three — IBC, HDPE Drum, MS Steel Drum.** Hard dependency for demo area 7 (see 10.48). Until they land, RM consumption cannot be modelled | RP |
| ⏳ 10.50 | **RM units of measure — expected to fall out of the BOMs.** Resin arrives in **25 kg bags** (SABIC, IOCL Propel) while 448/448 finished SKUs are `NOS`. The bridge is likely the drum's own weight: a printed drum reads `TARE WEIGHT : 7.80 KG`, and the item master's weight attribute is that same wall-thickness grade — implying near-1:1 kg-of-resin to drum. **Confirm against the BOM** | RP, obs-04 |
| ✅ 10.51 | **Auto-indent at re-order level: YES, with a configurable level (RP, 2026-08-21).** Ties areas 1, 2 and 6 into one continuous story. Note this is **new behaviour** — Supply Master has a `Re-order Level` field sitting unused at `0.00` | RP, extract |
| ✅ 10.52 | **Tally: NOT demonstrated live (RP, 2026-08-21).** Show **XML export buttons** only. Keeps the integration visible without staking the demo on it | RP |
| ✅ 10.53 | **Demo audience (RP, 2026-08-21): Gautam (IT), Jai (promoter), plant heads, Purchase, Sales, and Narayan Ji (head of the Bharuch cluster).** A mixed room — promoter wants trapped cash, plant heads want the production sheet, Gautam will ask about migration, Purchase and Sales want their own screens. **Narayan Ji is new to the project** and is the third senior name after Girdharlalji and Santoshi | RP |
| ✅ 10.12 | **Plant map ANSWERED 2026-08-21 (RP).** Bharuch: 3, 4, 5, 6, 7, 9. Silvassa: 1, 2. Wada: 8. Products — 3 and 4 HDPE; 5 and 7 HDPE+IBC; 6 MS steel drums; 9 recycling (separate GST, out of demo scope). Unit 8 = Khanivali/Wada is independently confirmed by GSTIN `27AACCP5074E3ZF`. **Supersedes the earlier 10.12 row** | RP, extract |
| ✅ 10.54 | **Demo plants DECIDED 2026-08-21 (RP): Units 6 and 7**, both Bharuch, assumed same GSTIN for the demo. Deliberately not 5+7 — **6 (MS steel drums) + 7 (HDPE + IBC) covers all three product lines across two sites**, and inter-plant movement travels on a delivery challan, not an invoice | RP |
| ✅ 10.55 | **All three product lines in the demo (RP, 2026-08-21)** — Plastic Barrels, MS Barrels, IBC. Implies three distinct production flows: blow moulding, steel forming, cage assembly | RP |
| ✅ 10.56 | **Demo opens on three problem statements (RP, 2026-08-21): (1) inventory visibility and tracking, (2) fleet management, (3) LR tracking.** Note this substitutes *inventory visibility/tracking* for the earlier *inventory ageing* pillar | RP |
| ✅ 10.57 | **Demo data is INVENTED (RP, 2026-08-21)** — except the BOMs, which will be real. No real vendor, customer or transaction data | RP |
| ✅ 10.58 | **Demo is 6 DAYS AWAY — 2026-08-27 (RP, 2026-08-21).** The binding constraint on scope | RP |
| ✅ 10.59 | **Units 6 and 7 share a GSTIN (RP, 2026-08-21)** — real, not just a demo simplification. Note Plant 9 (recycling, also Bharuch) is on a *separate* GST, so co-location does not imply a shared registration | RP, rec-32 |
| ✅ 10.60 | **Demo narrative spine: Sales Order → Dispatch, full loop (RP, 2026-08-21).** Read as: SO arrives → production planned → RM shortfall → indent → PO → LR → GRN → stock → produce → dispatch → invoice. Threads all 13 areas into one story rather than showing them as separate modules | RP |
| ✅ 10.61 | **Fleet in the demo: OWN TRUCKS ONLY (RP, 2026-08-21).** Contractors excluded, despite being real — R1 describes a contractual fleet, and the only complete outbound movement in evidence ran on *Anand Freight Carriers*. Simplifies the vehicle/driver model for the demo | RP |
| ✅ 10.62 | **One god-user for the demo (RP, 2026-08-21).** No role switching, no permission model. Note the audience includes Purchase, Sales and plant heads, who will each be watching for their own screen — worth naming roles in the narration even without enforcing them | RP |
| ✅ 10.63 | **LR alerting to the STORE TEAM is a must-have demo feature (RP, 2026-08-21).** Trigger per **stage**, not on total LR age; thresholds configurable; scoped to the destination plant. **Not routed to the fleet team** — they have no inbound role. Directly counters today's reactive discovery, and follows from 4.17 (no single dominant ageing stage, so instrument all of them) | RP |
...
3. ~~Capital trapped — need a number~~ ❌ **WITHDRAWN 2026-08-21 — the figure is not to be used. No verified number exists.**
