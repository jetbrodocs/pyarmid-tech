...
> ## ⚠️ Read the demo data policy
>
> **[`../demo-data-policy.md`](../demo-data-policy.md)** governs every rate, price, name and date that
> appears in a screen spec. **All demo figures are invented by us — nothing is being requested from
> Pyramid**, which is precisely why the discipline is written down.
>
> **These specs were written before the policy landed** (it was authored 2026-08-30 but sat unpushed
> until 2026-08-31). Its first rule is that **no monetary figure may be typed inline** — every one must
> reference the seed register in §4, so there is one place to change them. Specs naming figures
> directly need a pass to swap them for register references.
>
> Run the policy's §7 checklist over anything already written.
