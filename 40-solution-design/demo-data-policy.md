...
> ### Why 🔵 exists
>
> Tier 🟢 says _use exactly as documented_. Four places in the BOM workbooks **cannot** obey that,
...
**The boundary that matters most is inside a single screen.** A production run screen shows a _real_
BOM quantity (21.35 kg of resin) against an _invented_ rate (₹95/kg). Keep that line clear in your
head — see §5.
...
| **Real people's names** | All person names were stripped from the PRDs on 2026-08-30 and replaced with positions. **Do not reintroduce them.** Use _Plant Head_, _Store Head_, _Production Head_, _Shift Engineer_, _QA Engineer_, _Sales Team_, _Fleet Team_, _Purchase Team_ |
...
The claim is _"Phlo computes cost-to-serve per delivery."_ Never _"your cost-to-serve is ₹X."_
...
| **Spares & consumables (Path B — demo prd-01/prd-02)** |
| S1                                                     | Hydraulic seal kit                     | **₹150.00**    | per NOS               |
| S2                                                     | V-belt B-58                            | **₹450.00**    | per NOS               |
| S3                                                     | Corner protector                       | **₹40.00**     | per NOS               |
| S4                                                     | Screw with nylock nut 6×20             | **₹5.00**      | per NOS               |
| S5                                                     | Lubricant grease                       | **₹300.00**    | per kg                |

HSN codes on these rows are illustrative GST classifications for the item type (rubber seals,
belting, plastic mouldings, fasteners, lubricants) — real HSN chapter structure, not verified against
Pyramid's own filings. GST rate assumed **18%** uniformly for Path B spares in the demo, pending
Pyramid's actual slab per item.
...
|                  |                                                                                                                                                                                                                                                                                                                                                                                                  |
| ---------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **The conflict** | `CAGE-MAX` books `CUT VERTICAL BAR 1018` ×20 at **9,120 g** (456 g each — the _1002_ bar's weight). Level 2 defines 1018 as **463 g**. `CAGE-BIG` books the same part at 9,260 g, correctly. **140 g short per MAX cage.** Separately, `CAGE-MAX` consumes **both** 1018 ×20 **and** 1002 ×20 = 40 vertical bars, against BIG's 20                                                               |
...
| **The point**    | A2 and A3 look like one problem and are not. Collapsing A3 because the numbers match would be **reading an assumption as a fact** — the habit that produced the VP, the five product lines, and the inbound fleet. A2 has identical text _and_ a physical check; A3 has neither |
