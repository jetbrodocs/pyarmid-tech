...
| 7 | Production Planning (affecting RM) | [proc-04](proc-04-production.md) | 🔴 **planning** / 🟢 **execution** |
| 8 | Demand Planning | [proc-03](proc-03-sales-order-to-dispatch.md) Stage 0 | 🔴 Documented as **not existing** |
| 9 | Sales Orders | [proc-03](proc-03-sales-order-to-dispatch.md) | 🟢 screen / 🔴 process |
| 10 | Dispatch | [proc-03](proc-03-sales-order-to-dispatch.md) | 🟡 Sequencing rule known |
...
Three maps deliberately document an **absence** rather than a flow. Read them before assuming
coverage means knowledge:

| Map | What it actually says |
|---|---|
| `proc-03` Stage 0 | **There is no demand planning.** The promoters' judgement is the whole of it |
| `proc-04` Stage 1 | **Production planning is unknown.** Six sub-questions, all open. "Produce against SO" is a *demo assumption*, not a finding |
| `proc-06` | **Nothing is tracked today.** The document is a cost model, not an observed sequence |

Two maps carry an explicit evidence warning in their header — `proc-03` (*"do not read the numbered
flow as verified"*) and `proc-04` (execution well documented, planning unknown).
...
- **[Sales Order to Dispatch](proc-03-sales-order-to-dispatch.md)** — *demo areas 8, 9, 10, 11, 12.* Demand planning (**there is none**), order intake (**unknown**), Sales Order, dispatch sequencing by due date and SO age, fleet assignment, Delivery Challan → e-Way Bill → Sales Invoice → IRN. Exceptions: cancelled-order rework, cross-state split fulfilment.
- **[Production — Planning, Execution and Quality](proc-04-production.md)** — *demo area 7.* Planning **unknown**; execution well documented. Machine parameters, mould setup, QC gates, leak test, reject → serial deleted → granulation, customer-specific modification, serialisation.
